...
useHead({ title: "Cafe Pin — 내 카페 지도" });
...
function onSelect(id: number) {
  // 모바일 목록 탭 — 지도가 안 보이므로 선택 대신 기록 상세로 간다
  if (view.value === "list") {
    navigateTo(`/cafes/${id}`);
    return;
  }
  ...
  >
    ...
    >
      ...
      >
        ...
        <!-- 첫 실행 — 데스크톱은 왼쪽 목록의 빈 상태가 담당하므로 모바일 지도 뷰에서만 -->
        <div
          v-if="isEmptyLibrary && view === 'map'"
          class="absolute inset-0 z-20 flex items-center justify-center p-5 lg:hidden"
        >
          <div class="surface px-6 py-7">
            <UiEmptyState
              title="아직 기록한 카페가 없음"
              description="처음 기억하고 싶은 카페를 남겨보기"
            >
        ...
        <!-- 모바일 카페 추가 -->
        <NuxtLink
          v-if="!store.selected && !isEmptyLibrary"
          to="/cafes/new"
